...
module.exports = () => {
  ...
  router.get('/bigVideo', async (ctx) => {
    const stat = fs.statSync(path.join(__dirname, 'fflawbg.mp4'))
    const range = ctx.req.headers.range
    const parts = range.replace(/bytes=/, '').split('-')
    const start = Number(parts[0])
    const end = Number(parts[1]) || stat.size - 1
    ...
    ctx.set('Accept-Ranges', 'bytes')
    ctx.status = 206
    const stream = fs.createReadStream(path.join(__dirname, 'fflawbg.mp4'), {
      start,
      end
    })
    ctx.body = stream
  ...
  router.get('/backgroundVideoNormal', async (ctx) => {
    ...
    const video = fs.readFileSync(path.join(__dirname, 'fmp4.mp4'))
    ctx.type = 'video/mp4'
    ctx.body = video
